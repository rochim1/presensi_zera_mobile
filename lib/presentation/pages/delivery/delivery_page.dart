              ...
              builder: (context, state) {
                return AppTopBarActionButton(
                  onTap: () async {
                    ...
                    if (answer == null || !mounted || !context.mounted) return;
                    setState(() {
                            ...
                            onTap: () async {
                              ...
                              if (appState.hasCheckedOutToday && !appState.isOvertimeActive) {
                                await AppModalBottom.showDefault(
                                  ...
                                  contentSubtitle: 'Anda sudah pulang. Tekan tombol "Mulai Lembur" di Beranda terlebih dahulu jika ingin membuat Kunjungan tambahan.',
                                  hasActionPop: true,
                                );
                                return;
                              }
                              ...
                              if (presState.status.isLoaded && AppUtility.toBreakOut(presState.data)) {
                                await AppModalBottom.showDefault(
                                  context,
                                  emptyState: EmptyState.confirmation,
                                  contentTitle: 'Konfirmasi',
                                  contentSubtitle: 'Tidak bisa membuat Kunjungan, status kerja anda masih Istirahat',
                                  hasActionPop: true,
                                );
                                return;
                              }
                              ...
                              if (!ctxTask.mounted || !ctxUser.mounted || !ctxLocal.mounted) {
                                return;
                              }
                              
                              if (result ?? false) {
                                ctxTask.read<TasksGetAllPendingCubit>().initLoadAllData();
                                ctxUser.read<UserGetDataCubit>().getData(false);
                                ctxLocal.read<UserGetLocalCubit>().getData(false);
                                ctxTask.read<SalesTargetCubit>().loadMyTarget();
                              }
                            },
                          );
                        },
                    ...
                    >(
                      builder: (context, state) => Column(
                        children: [
                          ...
                          const Padding(
                            padding: EdgeInsets.symmetric(horizontal: AppDimens.paddingMediumX, vertical: AppDimens.paddingMedium),

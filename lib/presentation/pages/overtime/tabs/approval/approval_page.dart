...
@RoutePage()
class OvertimeRequestApprovalPage extends StatelessWidget {
  const OvertimeRequestApprovalPage({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => sl<OvertimeRequestApprovalCubit>()
        ..onChangeFilterDate(context.read<OvertimeCubit>().state.filterDate),
      child: const OvertimeRequestApprovalView(),
    );
  }
}

class OvertimeRequestApprovalView extends StatelessWidget {
  const OvertimeRequestApprovalView({super.key});
      ...
      listeners: [
        BlocListener<OvertimeCubit, OvertimeState>(
          ...
          listener: (context, state) {
            final cubit = context.read<OvertimeRequestApprovalCubit>();
        ...
        BlocListener<OvertimeCubit, OvertimeState>(
          ...
          listener: (context, state) {
            context.read<OvertimeRequestApprovalCubit>().onRefresh();
          },
        ),
      ],
      child:
              ...
              return AppInfiniteScrollView<OvertimeRequest>(
                ...
                loadingBuilder: (context) => SliverList.separated(
                  itemBuilder: (context, index) =>
                      const OvertimeRequestCard.shimmer(),
                  separatorBuilder: (context, index) => AppDimens.h12.hSpace,
                  itemCount: 5,
                ),
                sliversBefore: [
  ...
  Widget _buildApprovalActions(
    OvertimeRequestApprovalCubit cubit,

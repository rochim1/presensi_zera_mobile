...
class ReasonInput extends AppFormzInput<String?, ReasonValidationError>
  ...
  final LeaveApprovalAction approvalAction;

  const ReasonInput.pure({this.approvalAction = LeaveApprovalAction.none})
    : super.pure('');

  const ReasonInput.dirty(super.value, {required this.approvalAction})
    : super.dirty();
  ...
  ReasonValidationError? validator(String? value) {
    if (approvalAction.isNone) return null;
    final trimValue = value?.trim() ?? '';

    // LeaveApprovalAction.reject => required
    if (approvalAction.isReject) {
